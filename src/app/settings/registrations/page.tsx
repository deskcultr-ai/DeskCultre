...
export default function RegistrationsPage() {
  ...
  const [peopleAccess, setPeopleAccess] = useState(false); const [organizationAccess, setOrganizationAccess] = useState(false); const [reportsAccess, setReportsAccess] = useState(false); const [meetingsAccess, setMeetingsAccess] = useState(false); const [senior, setSenior] = useState(false);
  ...
  const canGrantManagerAccess = isAdminRole || isManagerRole;
  async function approve(request: Registration) {
    if (!company) return; setBusy(request.id); setError("");
    const { error: rpcError } = await supabase.rpc("approve_registration_request", { request_id: request.id, target_company_id: company, target_department_id: department || null, target_team_id: team || null, target_role: role, allow_task_creation: isAdminRole || create, allow_review: isAdminRole || review, allow_people: isAdminRole || (isManagerRole && peopleAccess), allow_organization: isAdminRole || (isManagerRole && organizationAccess), allow_reports: isAdminRole || (isManagerRole && reportsAccess), allow_meetings: isAdminRole || (isManagerRole && meetingsAccess), make_department_senior: isManagerRole && senior && Boolean(department) });
