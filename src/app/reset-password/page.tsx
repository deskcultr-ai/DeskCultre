...
export default function ResetPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setMessage(""); await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent("/account")}` }); setBusy(false); setMessage("If that address has an account, a password-reset link has been sent."); }
